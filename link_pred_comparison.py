...
def triadic_closure_pred_agent(node_pair, neighbors_collections, neighbors, \
                               directed, \
                               orig_colors, has_edge_types):
    (a, b) = node_pair
    shared_neighbors = neighbors[a] & neighbors[b]
    total_count = 0
    for n in shared_neighbors:
        total_count += int(n in neighbors_collections[a])
        total_count += int(n in neighbors_collections[b])
        total_count += int(a in neighbors_collections[n])
        total_count += int(b in neighbors_collections[n])
    return total_count

def k_hop_info_and_alternate_pred_agent(neighbors_collections, orig_colors, \
                                        directed, \
                                        has_edge_types, \
                                        true_entities, k, \
                                        ...
                                        alternate_pred_agent=triadic_closure_pred_agent):
    ...
    if num_processes > 1:
        ...
        (basic_edge_classes, positives_in_edge_class, observed_edges, \
            num_elements_by_score, num_positives_by_score) = \
        ...
        (basic_edge_classes, positives_in_edge_class, observed_edges, \
            num_alements_by_score, num_positives_by_score) = \
    ...
    alternate_classes = []
    for score, num_elements in num_elements_by_score.items():
        num_positives = num_positives_by_score[score]
        alternate_classes.append((-score, num_elements, num_positives))
    alternate_classes.sort()
    alternate_classes = [(ne, np) for (_, ne, np) in alternate_classes]
...
def __parallel_collection_function__(arg):
    ...
    num_elements_by_score = {}
    num_positives_by_score = {}
    ...
    for a in range(0, num_nodes):
        ...
        for b in range(a + int(not self_loops_in_true_edges), num_nodes):
            ...
            for (c, d) in ab_pairs:
                ...
                score = alternate_pred_agent((c, d), \
                                             neighbors_collections, \
                                             neighbors, directed, \
                                             orig_colors, has_edge_types)
                if score not in num_elements_by_score:
                    num_elements_by_score[score] = 0
                    num_positives_by_score[score] = 0
                num_elements_by_score[score] += 1
                if (c, d) in true_edges:
                    num_positives_by_score[score] += 1

                observed_edges += 1

                if "shortcut hack test" != "shortcut hack test":
                    EC = score

                elif k == "inf" or cfc or len(k_hop_nodes) == num_nodes:
...
def __parallel_aggregator__(results):
    (basic_edge_classes, positives_in_edge_class, observed_edges, \
        num_elements_by_score, num_positives_by_score) = results[0]
    results[0] = None

    for i in range(1, len(results)):
        (proc_bec, proc_piec, oe, nebs, npbs) = results[i]
        observed_edges += oe
        results[i] = None
        ...
        for score, num_elements in nebs.items():
            if score not in num_elements_by_score:
                num_elements_by_score[score] = 0
                num_positives_by_score[score] = 0
            num_elements_by_score[score] += num_elements
            num_positives_by_score[score] += npbs[score]
        del nebs
        del npbs
